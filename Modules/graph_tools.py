...
def is_two_graphs_isomorphic(connections_1, connections_2):
    ...
    if len(graph_1.vertices) != len(graph_2.vertices):
        error_1 = " Error!"
    print(
        "|" + same_size("# Vertices", 15) + same_size(str(len(graph_1.vertices)), 10) + same_size(
            str(len(graph_2.vertices)),
            10) + error_1)
    error_2 = ""
    ...
    if error_1 == "" and error_2 == "" or error_3 == "":
        list_of_graphs = []
        find_graph(list_of_graphs, [], [*range(1, len(graph_1.vertices) + 1)], [*range(1, len(graph_1.vertices) + 1)],
                   graph_1,
                   graph_2)
...
def inverse_permutation(line2):
    line1=[*range(1,len(line2)+1)]
    line_1 = write_permutation(line1)
    line_2 = write_permutation(line2)
    print("Permutation\nπ = / " + line_1 + " \\" +
          "\n    \\ " + line_2 + " /")
    new_line = [*range(1, len(line1) + 1)]
    for i in range(0, len(line1)):
        new_line[line2[i] - 1] = i + 1
    line_1 = write_permutation(line1)
    line_2 = write_permutation(new_line)
    print("Inverse\nπ^-1 = / " + line_1 + " \\" +
          "\n       \\ " + line_2 + " /")
    composition(line2, new_line)
    return new_line


def composition(line_1, line_2):
    if len(line_1) != len(line_2):
        print("Wrong arguments!")
        return False
    under_line = [*range(1, len(line_1) + 1)]
    for i in range(0, len(line_1)):
        under_line[i] = line_1[line_2[i] - 1]
    new_line_1 = write_permutation([*range(1, len(line_1) + 1)])
    new_line_2 = write_permutation(line_2)
    new_line_3 = write_permutation(under_line)
    print(
        "Composition\n            / " + new_line_1 + " \\" + "\nπ ◦ π^-1 = |  " + new_line_2 + "  |" + "\n            \\ " + new_line_3 + " /")


def write_permutation(line_given):
    line = ""
    for i in range(0, len(line_given)):
        if i != len(line_given) - 1:
            line = line + str(line_given[i]) + ", "
        else:
            line = line + str(line_given[i])
    return line


def permutation_of_graph(edges, line):
    old_edges = edges
    new_edges = double_copy_list(old_edges)
    for i in range(0, len(line)):
        for j in range(0, len(old_edges)):
            if i + 1 == old_edges[j][0]:
                new_edges[j][0] = line[i]
            elif i + 1 == old_edges[j][1]:
                new_edges[j][1] = line[i]
    print("Permutation of Graph")
    line_1 = write_permutation([*range(1, len(line) + 1)])
    line_2 = write_permutation(line)
    print("φ = / " + line_1 + " \\"
                              "\n    \\ " + line_2 + " /")
    print("Given Graph G = (" + str([*range(1, len(line) + 1)]) + ", " + str(
        old_edges) + ")\n" + "New Graph Gx = (" + str([*range(1, len(line) + 1)]) + ", " + str(new_edges) + ")")
    return new_edges


#inverse_permutation([2,3,4,1])
# composition([4,3,1,2],[4,1,2,3])
# a = [[1, 2], [1, 3], [2, 4], [3, 4], [1, 4]]
# graph1 = Graph(a)
# permutation_of_graph(graph1, [2, 3, 4, 1])
